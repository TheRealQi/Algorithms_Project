...
if __name__ == '__main__':
    ...
    m._win.title("A* Maze Solver")
    searchPath, aPath, fwdPath = aStar(m)
    a = agent(m, footprints=True, color=COLOR.cyan, filled=True, shape='square')
    b = agent(m, 1, 1, footprints=True, color=COLOR.blue, filled=True, goal=(m.rows, m.cols), shape='arrow')
    c = agent(m, footprints=True, color=COLOR.green, shape='square', filled=True)
